...
def payload():
    return {"object_kind": "push", "after": "abc123", "ref": "refs/heads/main", "project": {"id": 84366761, "path_with_namespace": "ericson-j-santos/reqsys-v2-enterprise-real"}}


class GitLabWebhookTests(unittest.TestCase):
    def setUp(self):
        self.queue = Queue()
        self.client = TestClient(create_app(self.queue, "gateway-secret", "hook-secret", "ericson-j-santos/reqsys-v2-enterprise-real"))
        self.headers = {"X-Gitlab-Token": "hook-secret", "X-Gitlab-Event": "Push Hook", "X-Gitlab-Event-UUID": "uuid-1"}
    ...
    def test_normalizes_supported_event(self):
        event = normalize("Push Hook", "uuid-1", payload(), "ericson-j-santos/reqsys-v2-enterprise-real")
        self.assertEqual(event.event_id, "uuid-1")
    ...
    def test_endpoint_rejects_invalid_payload_and_event(self):
