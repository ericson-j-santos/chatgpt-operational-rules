...
ENV_FILE = Path.home() / "AppData" / "Local" / "ReqSys" / "TodoGlobal24x7Noteri" / "runtime.env"
VENV = STATE / "venv"
...
def main() -> int:
    ...
    env.update({
        ...
        "WORKER_ID": "continuation-worker-Noteri",
        "ROUTER_PRIMARY_NODE": "DESKTOP-PDQK954",
        "ROUTER_SECONDARY_NODE": "Noteri",
        "ROUTER_HEALTHY_NODES": "DESKTOP-PDQK954,Noteri",
        "PYTHONPATH": str(ROOT),
    })
    log = LOG_FILE.open("a", encoding="utf-8")
    flags = subprocess.CREATE_NEW_PROCESS_GROUP | subprocess.DETACHED_PROCESS | subprocess.CREATE_NO_WINDOW
    proc = subprocess.Popen(
        [str(python_exe), "-m", "scripts.continuation_worker", "--interval-seconds", "5"],
        cwd=ROOT,
        env=env,
    ...
    if proc.poll() is not None:
        raise SystemExit(f"Noteri host worker exited rc={proc.returncode}")
    print(json.dumps({"status": "running", "pid": proc.pid, "node_id": "Noteri"}, sort_keys=True))
    return 0
