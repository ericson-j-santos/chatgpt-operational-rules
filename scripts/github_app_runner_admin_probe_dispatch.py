#!/usr/bin/env python3
"""Dispatch the allowlisted ReqSys GitHub App runner-admin diagnostic workflow."""
...
def main()->int:
    ...
    dispatched=run([gh,"workflow","run",WORKFLOW,"--repo",REPO,"--ref",REF,
                    "-f","enforce=false","-f","runner_admin_probe=false","-f","app_runner_admin_probe=true"])
    ...
    for _ in range(12):
        ...
        if listed.returncode==0:
            ...
            if rows:
                row=rows[0]
                print(json.dumps({"status":"dispatched","run_id":row.get("databaseId"),
                                  "run_url":row.get("url"),"head_sha":row.get("headSha"),
                                  "run_status":row.get("status"),"conclusion":row.get("conclusion"),
