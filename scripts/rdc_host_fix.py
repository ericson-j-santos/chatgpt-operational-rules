...
def inspect(path: Path) -> dict[str, object]:
    ...
    marker = "v5" if V5_MARKER in text else "v4" if V4_MARKER in text else "v3" if V3_MARKER in text else "v2" if V2_MARKER in text else "legacy"
    return {
        "path": str(path),
        "sha256": sha256_bytes(payload),
        "size": len(payload),
        "marker": marker,
        "already_fixed": marker in {"v3", "v4", "v5"},
