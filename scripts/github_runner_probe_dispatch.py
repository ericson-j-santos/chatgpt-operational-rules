#!/usr/bin/env python3
"""Dispatch the allowlisted ReqSys runner-admin diagnostic workflow branch."""
...
def main()->int:
    ...
    if not gh:
        ...
        return 3
    dispatched=run([gh,"workflow","run",WORKFLOW,"--repo",REPO,"--ref",REF,"-f","enforce=false","-f","runner_admin_probe=true"])
    if dispatched.returncode!=0:
        print(json.dumps({"status":"blocked","reason":"dispatch_failed","secret_values_exposed":False},sort_keys=True))
        return 4
    for _ in range(12):
        listed=run([gh,"run","list","--repo",REPO,"--workflow",WORKFLOW,"--branch",REF,"--event","workflow_dispatch","--limit","10","--json","databaseId,url,headSha,status,conclusion,createdAt"])
        if listed.returncode==0:
            ...
            if rows:
                row=rows[0]
                print(json.dumps({"status":"dispatched","run_id":row.get("databaseId"),"run_url":row.get("url"),"head_sha":row.get("headSha"),"run_status":row.get("status"),"conclusion":row.get("conclusion"),"secret_values_exposed":False},sort_keys=True))
                return 0
        time.sleep(2)
